...
do
  ...
  if echo "$output" | grep -q 'ResourceAlreadyExistsException'; then
    echo "SLO GetOwner99Availability already exists. Skipping creation."
    err=0
    break
  elif echo "$output" | grep -q 'InvalidParameterValue'; then
...
do
  ...
  err=$?
  if echo "$output" | grep -q 'ResourceAlreadyExistsException'; then
    echo "SLO GetOwnerP99Latency already exists. Skipping creation."
    err=0
    break
  elif echo "$output" | grep -q 'InvalidParameterValue'; then
    echo "Error creating SLO. Retrying attempt: $i"
...
do
  ...
  err=$?
  if echo "$output" | grep -q 'ResourceAlreadyExistsException'; then
    echo "SLO PostOwner99Availability already exists. Skipping creation."
    err=0
    break
  elif echo "$output" | grep -q 'InvalidParameterValue'; then
    echo "Error creating SLO. Retrying attempt: $i"
...
do
  ...
  err=$?
  if echo "$output" | grep -q 'ResourceAlreadyExistsException'; then
    echo "SLO PostOwnerP99Latency already exists. Skipping creation."
    err=0
    break
  elif echo "$output" | grep -q 'InvalidParameterValue'; then
    echo "Error creating SLO. Retrying attempt: $i"
